...
export type TRProviderMarkName = 'apple' | 'github' | 'google';
export type TRProviderMarkProps = ComponentProps<'svg'> & {
  provider: TRProviderMarkName;
};

export function TRProviderMark({
  ...
  return (
    <svg
      ...
      aria-label={ariaLabel}
      className={mergeClassNames('tr-provider-mark', className)}
      data-provider={provider}
      role={ariaLabel === undefined ? undefined : 'img'}
      viewBox="0 0 24 24"
    >
      {provider === 'google' ? <GooglePaths /> : null}
      {provider === 'github' ? <GithubPath /> : null}
      {provider === 'apple' ? <ApplePath /> : null}
    </svg>
  );
}
