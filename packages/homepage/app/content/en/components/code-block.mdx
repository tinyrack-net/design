...
`TRCodeBlock` ships no grammars. Supply a highlighter through the `highlighter` prop or `TRCodeHighlighterProvider`, and the set of valid `language` values follows from that choice. See [Syntax highlighting](/en/integrations/highlighter) for the setup paths.
