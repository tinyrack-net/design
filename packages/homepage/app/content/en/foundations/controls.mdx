...
description: "Choose a complete md or lg control recipe, align components across a task, and preserve every interaction state."
section: foundations
order: 6
---
...
Use `md` for the default compact operational screen and `lg` when touch comfort or task emphasis needs a larger target. Do not mix sizes in one action row merely to express visual hierarchy; use `intent` and `appearance` for that.
...
<div className="grid gap-3" role="list">{(['md', 'lg']).map((uiSize) => <div className="flex flex-wrap items-end gap-2" key={uiSize} role="listitem"><TRField.Root uiSize={uiSize}><TRField.Label>{uiSize} rack</TRField.Label><TRField.Control placeholder="rack-a" /></TRField.Root><TRButton uiSize={uiSize}>Deploy</TRButton></div>)}</div>
