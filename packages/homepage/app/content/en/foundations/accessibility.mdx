...
Keep help and error text connected to its field. Tinyrack's [Field guide](/en/web/components/field/) documents the label, description, error, and control composition. Do not put essential instructions only in a placeholder or tooltip.
...
Choose semantic pairs such as `--tinyrack-text` on `--tinyrack-surface`, `--tinyrack-on-primary` on `--tinyrack-primary`, and each status `*Foreground` on its matching surface. Test the rendered pair in every supported theme and state. For WCAG-oriented review, use at least `4.5:1` for normal text, `3:1` for large text, and `3:1` for essential control boundaries and state indicators.
...
- Forms and actions: [Field](/en/web/components/field/), [Button](/en/web/components/button/), [IconButton](/en/web/components/icon-button/), and [Link](/en/web/components/link/)
- Structured interaction: [Dialog](/en/web/components/dialog/), [Menu](/en/web/components/menu/), [Tabs](/en/web/components/tabs/), and [Table](/en/web/components/table/)
