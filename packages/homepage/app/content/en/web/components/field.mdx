...
<ComponentExampleTabs
  ...
  previewLayout="stretch"
  preview={<div className="grid gap-5 sm:grid-cols-2"><Stories.FieldPreview data-docs-example-item="" defaultValue="rack-medium" disabled={false} invalid={false} label="Medium field" size="md" /><Stories.FieldPreview data-docs-example-item="" defaultValue="rack-large" disabled={false} invalid={false} label="Large field" size="lg" /></div>}
  sources={[{ label: 'React', language: 'tsx', code: String.raw`import { TRField } from '@tinyrack/ui/components/field';
