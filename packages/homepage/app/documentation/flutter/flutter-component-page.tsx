  ...
  'inline-suggestions': {
    ...
    contractRows: [
      ...
      },
      {
        ...
        choices: {
          en: 'A multiline editor consumes the arrow and enter keys itself, so the host calls `controller.handleKeyEvent` first inside its own handler. Anything the list does not consume comes back as `KeyEventResult.ignored`, which leaves Enter-to-send intact.',
          ko: '여러 줄 편집기는 화살표와 Enter 키를 스스로 소비하므로, 호스트가 자신의 핸들러 안에서 `controller.handleKeyEvent`를 먼저 호출해요. 목록이 쓰지 않은 키는 `KeyEventResult.ignored`로 돌아오기 때문에 Enter로 보내기가 그대로 살아 있어요.',
          ja: '複数行エディタは矢印キーと Enter キーを自身で消費するため、ホストは自分のハンドラ内でまず `controller.handleKeyEvent` を呼びます。リストが消費しなかったキーは `KeyEventResult.ignored` として返るので、Enter での送信はそのまま機能します。',
        },
      },
      {
