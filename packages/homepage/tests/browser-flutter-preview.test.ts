...
const shardTests = [0, 1, 2, 3].map((shard) =>
  isBrowserAuditShardSelected(shard) ? it : it.skip,
);
const [t0, t1, t2, t3] = shardTests as [typeof it, typeof it, typeof it, typeof it];
...
describe('built Flutter Web component preview', () => {
  ...
  t0('updates Accordion content, multiple selection, and disabled items', async () => {
    ...
    try {
      ...
      const expectInteractiveValue = (expected: string) =>
        expect
          .poll(() => hasInteractiveValue(expected), { timeout: 10_000 })
          .toBe(true);
      const hasLatestMetricsArgument = (name: string, expected: unknown) =>
        page.evaluate(
            ...
            const latestMetrics = [...(messages ?? [])]
              ...
              .find(
                (message) =>
                  ...
                  (message as { component?: string }).component === 'accordion' &&
                  (message as { type?: string }).type === 'metrics',
              ) as { payload?: { args?: Record<string, unknown> } } | undefined;
            return latestMetrics?.payload?.args?.[argumentName] === expectedValue;
          },
          { argumentName: name, expectedValue: expected },
        );

      await clickFlutterPart('configureTrigger');
      await expectInteractiveValue('configure');
      ...
      await expectInteractiveValue('');
      await clickFlutterPart('trigger');
      await expectInteractiveValue('install');
      ...
      await expect
        .poll(() => hasLatestMetricsArgument('multiple', true), { timeout: 10_000 })
        .toBe(true);
      await clickFlutterPart('configureTrigger');
      await expectInteractiveValue('install,configure');
  ...
  t1(
    'loads one engine and synchronizes controls, reset, theme, and focus',
    async () => {
      const page = await browser.newPage({ viewport: { height: 900, width: 1280 } });
      await page.addInitScript(() => {
        const messages: unknown[] = [];
              ...
              page.evaluate(() => {
                ...
                return messages?.some(
                  (message) =>
                    typeof message === 'object' &&
                    message !== null &&
                    (message as { payload?: { theme?: string }; type?: string })
                      .type === 'stateChanged' &&
  ...
  t2(
    'drives every AnimatedNumber playground axis through the Flutter preview',
    async () => {
      const page = await browser.newPage({ viewport: { height: 900, width: 1280 } });
      await page.addInitScript(() => {
        const messages: unknown[] = [];
              ...
              page.evaluate(() => {
                ...
                return (messages ?? []).some(
                  (message) =>
                    ...
                    (message as { payload?: { args?: { value?: number } } }).payload
                      ?.args?.value === 10_000,
                );
              }),
            { timeout: 60_000 },
  ...
  t3('synchronizes AlertDialog label, disabled, and open controls', async () => {
    const page = await browser.newPage({ viewport: { height: 900, width: 1280 } });
    await page.addInitScript(() => {
      const messages: unknown[] = [];
      Object.defineProperty(window, '__flutterPreviewMessages', { value: messages });
  ...
  t0(
    'keeps a control value stable with a cached preview that omits request IDs',
    async () => {
      const page = await browser.newPage({ viewport: { height: 900, width: 1280 } });
      await page.addInitScript(() => {
        const messages: unknown[] = [];
  ...
  t1(
    'accepts a text field value interaction without accepting full state echoes',
    async () => {
      const page = await browser.newPage({ viewport: { height: 900, width: 1280 } });
      try {
  ...
  t2('keeps CheckboxGroup playground selections in sync and resets them', async () => {
    const page = await browser.newPage({ viewport: { height: 900, width: 1280 } });
    await page.addInitScript(() => {
      const messages: unknown[] = [];
      Object.defineProperty(window, '__flutterPreviewMessages', { value: messages });
